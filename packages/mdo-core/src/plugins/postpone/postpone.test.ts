...
import postpone from "./index";
import { runBlockTransform } from "../../testHelpers";

const { description, dependencies, getHook } = postpone;
...
describe("Plugin: Postpone", () => {
  ...
  describe("parse", () => {
    ...
    const runAt = (time: string, timezone: string, postpone: string) => {
      const block = {
        type: Block.TYPES.INCOMPLETE_TASK,
        text: "{{type}} hello {{postpone}}",
        postpone
      };
