...
const { description, dependencies, getHook } = start;
...
describe("Plugin: Start", () => {
  ...
  describe("parse", () => {
    ...
    const run = (time: string, timezone: string, start: string) => {
      const block = {
        type: Block.TYPES.INCOMPLETE_TASK,
        text: "{{type}} hello {{start}}",
        start
      };
      return runHook("parse", time, timezone, [block]);
    };
    ...
    describe("using time", () => {
      beforeEach(async () => {
        result = await run("2018-06-25T18:35-05", "America/Panama", "at 9pm");
      });
    ...
    describe('using "today"', () => {
      describe("with no time specified", () => {
        beforeEach(async () => {
          result = await run("2018-06-25T18:35", "America/Panama", "today");
        });
      ...
      describe("with a typo", () => {
        it("throws an error", done =>
          run("2018-06-25T18:35-05:00", "America/Panama", "troday at 8:30pm")
            .then(() => {
    ...
    describe('using "tomorrow"', () => {
      describe("with no time specified", () => {
        beforeEach(async () => {
          result = await run("2018-06-25T18:35", "America/Panama", "tomorrow");
        });
      ...
      describe("with a typo", () => {
        it("throws an error", done =>
          run("2018-06-25T18:35-05:00", "America/Panama", "tomorrows at 8:30pm")
            .then(() => {
              return done(new Error("Expected transform to throw an error."));
            })
            .catch(err => {
    ...
    describe("using a specific date", () => {
      ...
      describe("when the date provided is invalid", () => {
        it("raises an error", done =>
          run("2018-06-25T18:35-05:00", "America/Panama", "2018-07-32")
            .then(() => {
              return done(new Error("Expected transform to throw an error."));
            })
            .catch(err => {
      ...
      describe("when the time provided is invalid", () => {
        it("raises an error", done =>
          run("2018-06-25T18:35-05:00", "America/Panama", "2018-07-30 at 13pm")
            .then(() => {
              return done(new Error("Expected transform to throw an error."));
            })
            .catch(err => {
    ...
    describe('using month or year without specifying "in" or "next"', () => {
      it("raises an error", done =>
        run("2018-06-25T18:35-05:00", "America/Panama", "year")
          .then(() => {
            return done(new Error("Expected transform to throw an error."));
          })
          .catch(err => {
  ...
  describe("stringify", () => {
    const run = (start: string) => {
      ...
      return runHook("stringify", "2019-01-01T00:00:00Z", "America/Panama", [
        block
      ]);
    };
